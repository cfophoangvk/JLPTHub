        ...
        <c:if test="${not empty param.success}">
            <div class="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4"
                ...
                <span class="block sm:inline">
                    <c:choose>
                        <c:when test="${param.success == 'created'}">Thêm điểm ngữ pháp thành công!</c:when>
                        <c:when test="${param.success == 'updated'}">Cập nhật điểm ngữ pháp thành công!</c:when>
                        <c:when test="${param.success == 'deleted'}">Xóa điểm ngữ pháp thành công!</c:when>
                                ...
                                <c:choose>
                                    <c:when test="${not empty gp.explanation}">
                                        <span class="truncate max-w-xs block" title="${gp.explanation}">
                                            ${gp.explanation.length() > 50 ? gp.explanation.substring(0, 50).concat('...') : gp.explanation}
                                        </span>
                                    </c:when>
                                    <c:otherwise>
                                        <span class="text-gray-400 italic">Không có</span>
                                ...
                                <c:choose>
                                    <c:when test="${not empty gp.example}">
                                        <span class="truncate max-w-xs block" title="${gp.example}">
                                            ${gp.example.length() > 50 ? gp.example.substring(0, 50).concat('...') : gp.example}
                                        </span>
                                    </c:when>
                                    <c:otherwise>
                                        <span class="text-gray-400 italic">Không có</span>
